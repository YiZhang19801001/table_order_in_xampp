...
class ModelCheckoutOrder extends Model {
	public function addOrder($data) {
		...
		$this->db->query("INSERT INTO `" . DB_PREFIX . "order` SET invoice_prefix = '" . $this->db->escape($data['invoice_prefix']) . "', store_id = '" . (int)$data['store_id'] . "', store_name = '" . $this->db->escape($data['store_name']) . "', store_url = '" . $this->db->escape($data['store_url']) . "', customer_id = '" . (int)$data['customer_id'] . "', customer_group_id = '" . (int)$data['customer_group_id'] . "', firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', fax = '" . $this->db->escape($data['fax'])  . "', payment_firstname = '" . $this->db->escape($data['payment_firstname']) . "', payment_lastname = '" . $this->db->escape($data['payment_lastname']) . "', payment_address_1 = '" . $this->db->escape($data['payment_address_1']) . "', payment_address_2 = '" . $this->db->escape($data['payment_address_2']) . "', payment_country = '" . $this->db->escape($data['payment_country']) . "', payment_country_id = '" . (int)$data['payment_country_id'] . "', payment_state = '" . $this->db->escape($data['payment_state']) . "', payment_state_id = '" . (int)$data['payment_state_id']. "', payment_suburb = '" . $this->db->escape($data['payment_suburb']) . "', payment_suburb_id = '" . (int)$data['payment_suburb_id'] . "', payment_method = '" . $this->db->escape($data['payment_method']) . "', payment_code = '" . $this->db->escape($data['payment_code']) . "', shipping_firstname = '" . $this->db->escape($data['shipping_firstname']) . "', shipping_lastname = '" . $this->db->escape($data['shipping_lastname']) . "', shipping_email = '" . $this->db->escape($data['shipping_email']) . "', shipping_telephone = '" . $this->db->escape($data['shipping_telephone']) . "', shipping_address_1 = '" . $this->db->escape($data['shipping_address_1']) . "', shipping_address_2 = '" . $this->db->escape($data['shipping_address_2']) . "', shipping_country = '" . $this->db->escape($data['shipping_country']) . "', shipping_country_id = '" . (int)$data['shipping_country_id'] . "', shipping_state = '" . $this->db->escape($data['shipping_state']) . "', shipping_state_id = '" . (int)$data['shipping_state_id'] . "', shipping_suburb = '" . $this->db->escape($data['shipping_suburb']) . "', shipping_suburb_id = '" . (int)$data['shipping_suburb_id'] . "', shipping_orderTime = '" . $this->db->escape($data['shipping_orderTime']). "', shipping_orderDate = '" . $this->db->escape($data['shipping_orderDate']). "', shipping_orderWhen = '" . $this->db->escape($data['shipping_orderWhen'])  . "', shipping_method = '" . $this->db->escape($data['shipping_method']) . "', comment = '" . $this->db->escape($data['comment']) . "', total = '" . (float)$data['total'] . "', affiliate_id = '" . (int)$data['affiliate_id'] . "', commission = '" . (float)$data['commission'] . "', marketing_id = '" . (int)$data['marketing_id'] . "', tracking = '" . $this->db->escape($data['tracking']) . "', language_id = '" . (int)$data['language_id'] . "', currency_id = '" . (int)$data['currency_id'] ."' , currency_code = '" . $this->db->escape($data['currency_code']) . "', currency_value = '" . (float)$data['currency_value'] . "', ip = '" . $this->db->escape($data['ip']) . "', forwarded_ip = '" .  $this->db->escape($data['forwarded_ip']) . "', user_agent = '" . $this->db->escape($data['user_agent']) . "', accept_language = '" . $this->db->escape($data['accept_language']) . "', date_added = NOW(), date_modified = NOW()");
		...
		if (isset($data['products'])) {
			...
			foreach ($data['products'] as $product) {
				...
				//ext
				if(!empty($product['exts'])){

					foreach ($product['exts'] as $ext) {
						$this->db->query("INSERT INTO " . DB_PREFIX . "order_ext SET product_ext_id = '".(int)$ext['product_ext_id']."', product_id ='".(int)$product['product_id'] . "', order_product_id ='".(int)$order_product_id."'");
					}
				}
			}
		}
		...
		if(isset($data['coupon'])) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "order_coupon SET order_id = '" . (int)$order_id . "', name = '" . $this->db->escape($data['coupon']['name']) . "', type = '" . (int)$data['coupon']['type'] . "', info = '" . $this->db->escape($data['coupon']['info']) . "', amount = '" . (float)$data['coupon']['amount'] ."'");
		}
	...
	public function editOrder($order_id, $data) {
		...
		//send email
		if (isset($data['order_info']['email']) && (int)$data['order_status_id'] == 4 ) {
		...
		if (isset($data['order_info']['email']) && (int)$data['order_status_id'] == 5 ) {
	...
	public function getOrder($order_id) {
		...
		if ($order_query->num_rows) {
			...
			return array(
				...
				'shipping_state'           => $order_query->row['shipping_state'],
				'shipping_country_id'     => $order_query->row['shipping_country_id'],
				'shipping_country'        => $order_query->row['shipping_country'],				
				'shipping_suburb_id'     => $order_query->row['shipping_suburb_id'],
				'shipping_suburb'        => $order_query->row['shipping_suburb'],
	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false) {
		...
		if ($order_info) {
			...
			// 	$message .= "Congratulations! Your Order ".$order_id." has been placed.\n";
			// 	$message .= "Your order will be ready by the estimated time ". date('Y-m-d H:i:s',time()).".\n";
			// 	$message .= "Regards\n";
