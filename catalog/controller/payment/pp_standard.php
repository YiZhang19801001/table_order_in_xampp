...
class ControllerPaymentPPStandard extends Controller {
	public function index() {
		...
		if (!empty($location_id)) {
			...
			if (!$this->config->get('pp_standard_test')) {
				$data['action'] = 'https://www.paypal.com/cgi-bin/webscr?locale.x=en_GB';
			} else {
				$data['action'] = 'https://www.sandbox.paypal.com/cgi-bin/webscr?locale.x=en_GB';
			}
			...
			if ($order_info) {
				...
				$total = $this->currency->format($order_info['total'] - $this->cart->getSubTotal($location_id), $order_info['currency_code'], false, false);
				...
				$data['total_text'] = $this->currency->format($order_info['total']);
				...
				$data['return'] = $this->url->link('pay/success');
				$data['notify_url'] = $this->url->link('payment/pp_standard/callback', '', 'SSL');
				$data['cancel_return'] = $this->url->link('pay/checkout', '', 'SSL');
	...
	public function callback() {
		...
		$file  = 'log.txt';
		$content = $this->request->post['payment_status']."\n";
