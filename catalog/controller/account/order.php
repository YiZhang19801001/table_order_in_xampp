...
class ControllerAccountOrder extends Controller {
	...
	public function index() {
		...
		foreach ($results as $result) {
			...
			$store_info = $this->model_localisation_location->getLocation($result['store_id']);
			$store_pic = "";
			if(is_file(DIR_IMAGE . $store_info['image'])){
				$store_pic = $this->model_tool_image->resize($store_info['image'], 140, 140);
			}

			$products = array();

			$products_info = $this->model_account_order->getOrderProducts($result['order_id']);

			foreach ($products_info as $k => $v) {

				$option_info = $this->model_account_order->getOrderOptions($v['order_id'],$v['order_product_id']);

				if($option_info){
					$option_name = $option_info['value'];
				}else{
					$option_name= "";
				}
			...
			$review_info = $this->model_shop_reviews->getReviewsByOrderId($result['order_id']);

			$reviews = array();
			if(!empty($review_info)) {

				$avg_rating = round(($review_info['rating']+$review_info['rating1']+$review_info['rating2'])/3,1);

				$reviews = array(
					'date_add' => $review_info['date_added'],
					'avg_rating' => $avg_rating,
					'rating' => floatval($review_info['rating']),
					'rating1' => floatval($review_info['rating1']),
					'rating2' => floatval($review_info['rating2']),
				);
			}
			...
			$data['orders'][] = array(
				...
				"store_pic" => $store_pic,
				"store_url" => $this->url->link('shop/info','location_id='.$result['store_id']),
				"date_added" => $result['date_added'],
				"order_status" => $order_status,
				"products" => $products,
				"total" => $this->currency->format($result['total']),
				"reorder"=>$this->url->link('pay/checkout','reorder='.$result['order_id']),
				'review_info' => $reviews,
				'review_url' => $this->url->link('shop/reviews','location_id='.$result['store_id']."&order_id=".$result['order_id'])
			);
		}
