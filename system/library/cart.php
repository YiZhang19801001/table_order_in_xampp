...
class Cart
{
    ...
    public function getProducts($l_id)
    {
        if (!$this->data) {
                ...
                if ($l_id == $location_id) {
                    ...
                    if ($product_query->num_rows) {
                        ...
                        if (!empty($exts)) {
                            ...
                            $product_exts = $this->db->query("SELECT * FROM `" . DB_PREFIX . "product_ext`  WHERE  `product_ext_id` in (" . $exts . ") ");
                            if ($product_exts->num_rows) {
                                $product_ext = $product_exts->rows;

                                foreach ($product_ext as $v) {

                                    $query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "product_add_type` WHERE  add_type_id = '" . (int) $v['type'] . "'");
                                    if ($query->row['type']) {
                                        $add_price += $v['price'];
                                    }
                                }
                        ...
                        $this->data[$key] = array(
                            ...
                            'total'           => ($price + $option_price + $add_price) * $quantity,
                            ...
                            'recurring'       => $recurring,
                            'ext'             => $product_ext,
                            'is_discount'     => $product_query->row['is_discount'],
                        );
                    } else {
                        $this->remove($key);
                    }
    ...
    public function getRecurringProducts($l_id)
    {
        $recurring_products = array();

        foreach ($this->getProducts($l_id) as $key => $value) {
            if ($value['recurring']) {
    ...
    public function add($product_id, $qty = 1, $option = array(), $location_id, $ext = "")
    {
        $this->data = array();
        ...
        if (!empty($qty)) {
            ...
            if (!isset($this->session->data['cart'][$key])) {
                ...
                if ($qty > 0 || $this->session->data['cart'][$key] > (-$qty)) {
                    $this->session->data['cart'][$key] += (int) $qty; //add
                } else {
                    unset($this->session->data['cart'][$key]);
                }
            }

            if (!empty($this->session->data['cart'][$key]) && intval($this->session->data['cart'][$key]) <= 0) {
                unset($this->session->data['cart'][$key]);
            }
        }
    }
    ...
    public function getWeight($l_id = 0)
    {
        $weight = 0;

        foreach ($this->getProducts($l_id) as $product) {
            if ($product['shipping']) {
    ...
    public function getSubTotal($l_id = 0)
    {
        $total = 0;
    ...
    public function getTaxes($l_id = 0)
    {
        $tax_data = array();
    ...
    public function getTotal($l_id = 0)
    {
        $total = 0;
    ...
    public function countProducts($l_id = 0)
    {
        $product_total = 0;
    ...
    public function hasProducts($l_id = 0)
    {

        return count($this->getProducts($l_id));

    }

    public function hasRecurringProducts($l_id = 0)
    {
        return count($this->getRecurringProducts($l_id));
    }

    public function hasStock($l_id = 0)
    {
        $stock = true;
    ...
    public function hasShipping($l_id = 0)
    {
        $shipping = false;
