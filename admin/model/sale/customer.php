...
class ModelSaleCustomer extends Model {
	...
	public function getAddress($address_id) {
		...
		if ($address_query->num_rows) {
			...
			$state_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "state` WHERE state_id = '" . (int)$address_query->row['state_id'] . "'");

			if ($state_query->num_rows) {
				$state = $state_query->row['name'];
			} else {
				$state = '';
			}

			$suburb_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "suburb` WHERE suburb_id = '" . (int)$address_query->row['suburb_id'] . "'");

			if ($suburb_query->num_rows) {
				$suburb = $suburb_query->row['name'];
			} else {
				$suburb = '';
			}
			...
			return array(
				...
				'state_id'        => $address_query->row['state_id'],
				'state'           => $state,
				'suburb_id'        => $address_query->row['suburb_id'],
				'suburb'           => $suburb,
