...
class ModelLocalisationTaxClass extends Model {
	public function addTaxClass($data) {
		...
		if ($data['approve']) {
			$this->db->query("UPDATE " . DB_PREFIX . "tax_class SET approve = 0 ");
		}

		$this->db->query("INSERT INTO " . DB_PREFIX . "tax_class SET title = '" . $this->db->escape($data['title']) . "', approve = '" . (int)$data['approve'] . "', description = '" . $this->db->escape($data['description']) . "', date_added = NOW()");
	...
	public function editTaxClass($tax_class_id, $data) {
		...
		$this->db->query("UPDATE " . DB_PREFIX . "tax_class SET title = '" . $this->db->escape($data['title']) . "', approve = '" . (int)$data['approve'] . "', description = '" . $this->db->escape($data['description']) . "', date_modified = NOW() WHERE tax_class_id = '" . (int)$tax_class_id . "'");
	...
	public function getDefaultTaxClass() {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "tax_class WHERE approve = 1");
