...
class ModelLocalisationGeoZone extends Model {
	public function addGeoZone($data) {
		...
		if (isset($data['zone_to_geo_zone'])) {
			foreach ($data['zone_to_geo_zone'] as $value) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "zone_to_geo_zone SET country_id = '" . (int)$value['country_id'] . "', state_id = '" . (int)$value['state_id'] . "', suburb_id = '" . (int)$value['suburb_id']. "', fee = '" . (int)$value['fee'] . "', geo_zone_id = '" . (int)$geo_zone_id . "', date_added = NOW()");
			}
		}
	...
	public function editGeoZone($geo_zone_id, $data) {
		...
		if (isset($data['zone_to_geo_zone'])) {
			foreach ($data['zone_to_geo_zone'] as $value) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "zone_to_geo_zone SET country_id = '" . (int)$value['country_id'] . "', state_id = '" . (int)$value['state_id'] . "', suburb_id = '" . (int)$value['suburb_id']. "', fee = '" . (int)$value['fee']. "', sort_order = '" . (int)$value['sort_order'] . "', geo_zone_id = '" . (int)$geo_zone_id . "', date_added = NOW()");
			}
		}
	...
	public function getZoneToGeoZones($geo_zone_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "zone_to_geo_zone WHERE geo_zone_id = '" . (int)$geo_zone_id . "' ORDER BY sort_order DESC");
