			...
			<div class="panel-body">
				<form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form-product" class="form-horizontal">
					<ul class="nav nav-tabs">
						...
						<li><a href="#tab_ext" data-toggle="tab"><?php echo $tab_ext; ?></a></li>
					</ul>
						...
						<div class="tab-pane" id="tab-data">
							...
							<div class="form-group">
								...
								<div class="col-sm-10">
									<label class="radio-inline">
										<?php if ($is_discount) { ?>
										<input type="radio" name="is_discount" value="1" checked="checked" />
										<?php echo $text_yes; ?>
										<?php } else { ?>
									...
									<label class="radio-inline">
										<?php if (!$is_discount) { ?>
										<input type="radio" name="is_discount" value="0" checked="checked" />
										<?php echo $text_no; ?>
										<?php } else { ?>
...
<!-- 							<div class="form-group">
							...
							<div class="form-group">
								<label class="col-sm-2 control-label" for="input-product_tag"><?php echo $entry_ptag; ?></label>
								<div class="col-sm-10">
									<input type="text" name="product_tag" value="" placeholder="<?php echo $entry_ptag; ?>" id="input-product_tag" class="form-control" />
									<div id="product-tag" class="well well-sm" style="height: 150px; overflow: auto;">
										<?php foreach ($product_tags as $product_tag) { ?>
										<div id="product-tag<?php echo $product_tag['product_tag_id']; ?>"><i class="fa fa-minus-circle"></i> <?php echo $product_tag['name']; ?>
											<input type="hidden" name="product_tags[]" value="<?php echo $product_tag['product_tag_id']; ?>" />
										</div>
										<?php } ?>
									</div>
								</div>
...
<!-- 							<div class="form-group">
						...
						<div class="tab-pane" id="tab_ext">
							<div class="table-responsive">
								<table id="ext" class="table table-striped table-bordered table-hover">
									<thead>
										<tr>
											<!-- <td class="text-left"><?php echo $entry_option_value; ?></td> -->
									...
									<tbody>
										<?php $ext_row = 0; ?>
										<?php foreach ($product_exts as $product_ext) { ?>
										<tr id="ext-row<?php echo $ext_row; ?>">
											<td class="text-right">
												<select name="product_ext[<?php echo $ext_row; ?>][type]" class="form-control">

													<?php foreach($types as $type){?>
													<option value="<?php echo $type['add_type_id']?>" <?php if($product_ext['type'] == $type['add_type_id']){?>selected="selected"<?php }?>><?php echo $type['name']; ?></option>
													<?php }?>
												
												</select>
											</td>
											<td class="text-right">
												<input type="text" name="product_ext[<?php echo $ext_row; ?>][name]" value="<?php echo $product_ext['name']; ?>" placeholder="<?php echo $entry_name; ?>" class="form-control" />
											</td>
											<td class="text-right">
												<input type="text" name="product_ext[<?php echo $ext_row; ?>][price]" value="<?php echo $product_ext['price']; ?>" placeholder="<?php echo $entry_price; ?>" class="form-control" />
											</td>
											<td class="text-left">
												<button type="button" onclick="$('#ext-row<?php echo $ext_row; ?>').remove();" data-toggle="tooltip" title="<?php echo $button_remove; ?>" class="btn btn-danger"><i class="fa fa-minus-circle"></i></button>
											</td>
										</tr>
										<?php $ext_row++; ?>
										<?php } ?>
									...
									<tfoot>
										<tr>
											<td colspan="3"></td>
											<td class="text-left"><button type="button" onclick="addExt();" data-toggle="tooltip" title="<?php echo $button_discount_add; ?>" class="btn btn-primary"><i class="fa fa-plus-circle"></i></button></td>
										</tr>
									</tfoot>
								</table>
							</div>
...
$('input[name=\'product_tag\']').autocomplete({
	'source': function(request, response) {
		$.ajax({
			url: 'index.php?route=catalog/product_tag/autocomplete&token=<?php echo $token; ?>&filter_name=' +  encodeURIComponent(request),
			dataType: 'json',     
			success: function(json) {
				response($.map(json, function(item) {
					return {
	...
	'select': function(item) {
		...
		$('#product-tag').append('<div id="product-tag' + item['value'] + '"><i class="fa fa-minus-circle"></i> ' + item['label'] + '<input type="hidden" name="product_tags[]" value="' + item['value'] + '" /></div>'); 
	}
});
...
//--></script> 	
<script type="text/javascript"><!--
var ext_row = <?php echo $ext_row; ?>;

function addExt() {
	html  = '<tr id="ext-row' + ext_row + '">';
	html += '  <td class="text-right"><select name="product_ext[' + ext_row + '][type]" class="form-control">';
	<?php foreach ($types as $type) { ?>
	html += '    <option value="<?php echo $type['add_type_id']; ?>"><?php echo addslashes($type['name']); ?></option>';
	<?php } ?>
	html += '  </td>';
	html += '  <td class="text-right"><input type="text" name="product_ext[' + ext_row + '][name]" value="" placeholder="<?php echo $entry_name; ?>" class="form-control" /></td>';
	html += '  <td class="text-right"><input type="text" name="product_ext[' + ext_row + '][price]" value="" placeholder="<?php echo $entry_price; ?>" class="form-control" /></td>';	
	html += '  <td class="text-left"><button type="button" onclick="$(\'#ext-row' + ext_row + '\').remove();" data-toggle="tooltip" title="<?php echo $button_remove; ?>" class="btn btn-danger"><i class="fa fa-minus-circle"></i></button></td>';
	html += '</tr>';	
	
	$('#ext tbody').append(html);
