...
class ControllerCatalogProduct extends Controller {
	...
	protected function getForm() {
		...
		$data['entry_ptag'] = $this->language->get('entry_ptag');
		...
		$data['tab_ext'] = $this->language->get('tab_ext');
		...
		$approve = $this->model_localisation_tax_class->getDefaultTaxClass();
		...
		if (isset($this->request->post['tax_class_id'])) {
			...
			//默认
			if ($approve) {
				$data['tax_class_id'] = $approve['tax_class_id'];
			} else {
				$data['tax_class_id'] = 0;
			}
		}
		...
		if (isset($this->request->post['is_discount'])) {
			$data['is_discount'] = $this->request->post['is_discount'];
		} elseif (!empty($product_info)) {
			$data['is_discount'] = $product_info['is_discount'];
		} else {
			$data['is_discount'] = 1;
		}
		...
		if (isset($this->request->post['manufacturer'])) {
			...
			$manufacturer_info = $this->model_localisation_location->getLocation($product_info['manufacturer_id']);
		...
		if (isset($this->request->post['product_tags'])) {
			$data['product_tags'] = $this->request->post['product_tags'];
		} elseif (isset($this->request->get['product_id'])) {
			$product_tag = $this->model_catalog_product_tag->getProductTagsByProductId($this->request->get['product_id']);
			$product_tags = array();
			if(!empty($product_tag)) {

				$product_tag = explode(',', $product_tag);
				if(!empty($product_tag)) {
					foreach ($product_tag as $v) {
						$product_tags[] = $this->model_catalog_product_tag->getProductTag($v);
					}
				}
			}
		...
		//ext
		if (isset($this->request->post['product_ext'])) {
			$data['product_exts'] = $this->request->post['product_ext'];
		} elseif (isset($this->request->get['product_id'])) {
			$data['product_exts'] = $this->model_catalog_product->getProductExt($this->request->get['product_id']);

		} else {
			$data['product_exts'] = array();
		}

		//option_ext
		$this->load->model('catalog/add_type');

		$data['types'] = $this->model_catalog_add_type->getProductAddTypes();
