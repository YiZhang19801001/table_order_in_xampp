...
class ControllerSaleOrder extends Controller {
	...
	public function getForm() {
		...
		$data['entry_state_code'] = $this->language->get('entry_state_code');
		...
		if (!empty($order_info)) {
			...
			$data['payment_state_id'] = $order_info['payment_state_id'];
			$data['payment_suburb_id'] = $order_info['payment_suburb_id'];
			...
			$data['shipping_state_id'] = $order_info['shipping_state_id'];
			$data['shipping_suburb_id'] = $order_info['shipping_suburb_id'];
			...
			foreach ($products as $product) {
				...
				$exts = $this->model_sale_order->getOrderExts($product['order_product_id']);
				
				foreach ($exts as $ext) {
					$product_ext = $this->model_sale_order->getOrderExtInfo($ext['product_ext_id']);
					$type_text = "";

					if ($product_ext['type']) {
						$type_array = $this->model_sale_order->getOrderType($product_ext['type']);

						$type_text = $type_array['name'];
					}
					$ext_data[] = array(
						'name'  => $product_ext['name'],
						'type'  => $type_text,
						'product_ext_id'=>$ext['product_ext_id'],
						'price' => $this->currency->format($product_ext['price'])
					);
				}
	...
	public function info() {
		...
		if ($order_info) {
			...
			$data['text_state_code'] = $this->language->get('text_state_code');
			$data['text_country'] = $this->language->get('text_country');
			$data['text_suburb'] = $this->language->get('text_suburb');
			$data['text_shipping_method'] = $this->language->get('text_shipping_method');
			$data['text_payment_method'] = $this->language->get('text_payment_method');
			$data['text_shipping_orderTime'] = $this->language->get('text_shipping_orderTime');
			$data['text_shipping_orderDate'] = $this->language->get('text_shipping_orderDate');
			$data['text_shipping_orderWhen'] = $this->language->get('text_shipping_orderWhen');
			...
			if ($order_info['customer_id']) {
				$this->load->model('sale/customer');
				$data['customer_info'] = $this->model_sale_customer->getCustomer($order_info['customer_id']);

				if (isset($data['customer_info']['address_id'])) {
					$data['address'] = $this->model_sale_customer->getAddress($data['customer_info']['address_id']);
				}
			...
			//coupon
			$data['coupon_info'] = $this->model_sale_order->getCouponByOrderId($order_id);
			if (!empty($data['coupon_info']) && ($data['coupon_info']['type'] == 1 || $data['coupon_info']['type'] == 3) && $data['coupon_info']['amount'] == 0) {
				$data['coupon_info'] = "";
			}

			if ($order_info['customer_id']) {
				$times = $this->model_sale_order->getTotalOrdersTime($order_info['customer_id']);
				$totalPrice = 0;
				$m = $times % 9;
				$times =  8 - $m;
				
				$query = $this->db->query("SELECT total FROM `" . DB_PREFIX . "order` WHERE customer_id = '" . (int)$order_info['customer_id'] . "' AND order_status_id = 3  ORDER BY order_id DESC LIMIT 0,".(int)$m);
				...
				if ($query->num_rows) {
					...
					$totalPrice = $query_total / 8; 
				}

				$data['coupon_info_text'] = "{$this->currency->format($totalPrice)}  total ;  {$times} orders to go";
			}
			...
			$data['payment_address_1'] = $order_info['payment_address_1'];
			$data['payment_address_2'] = $order_info['payment_address_2'];
			$data['payment_state'] = $order_info['payment_state'];
			$data['payment_country'] = $order_info['payment_country'];
			$data['payment_suburb'] = $order_info['payment_suburb'];
			...
			$data['shipping_state'] = $order_info['shipping_state'];
			$data['shipping_country'] = $order_info['shipping_country'];
			$data['shipping_suburb'] = $order_info['shipping_suburb'];
			$data['shipping_orderTime'] = $order_info['shipping_orderTime'];
			$data['shipping_orderDate'] = $order_info['shipping_orderDate'];
			$data['shipping_orderWhen'] = $order_info['shipping_orderWhen'];
	...
	public function country() {
		...
		if ($country_info) {
			...
			$json = array(
				...
				'state'              => $this->model_localisation_state->getStatesByCountryId($this->request->get['country_id']),
	...
	public function state() {
		...
		if ($state_info) {
			...
			$json = array(
				...
				'suburb'              => $this->model_localisation_suburb->getSuburbByStateId($this->request->get['state_id']),
