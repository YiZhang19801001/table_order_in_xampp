...
class ControllerLocalisationGeoZone extends Controller {
	...
	public function state() {
		...
		$results = $this->model_localisation_state->getStatesByCountryId($this->request->get['country_id']);

		foreach ($results as $result) {
			$output .= '<option value="' . $result['state_id'] . '"';
			if ($this->request->get['state_id'] == $result['state_id']) {
	...
	public function suburb() {
		...
		$results = $this->model_localisation_suburb->getSuburbByStateId($this->request->get['state_id']);

		foreach ($results as $result) {
			$output .= '<option value="' . $result['suburb_id'] . '"';
			if ($this->request->get['suburb_id'] == $result['suburb_id']) {
				$output .= ' selected="selected"';
			}
			$output .= '>' . $result['name'] . '</option>';
		}
